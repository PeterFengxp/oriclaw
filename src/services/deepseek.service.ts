...
/**
 * DeepSeek service implementation for multimodal AI capabilities
 * Supports DeepSeek multimodal model
 */
export class DeepSeekService implements ILLMProvider {
  private apiKey: string;
  private baseUrl: string = 'https://api.deepseek.com/v1';
  private conversationHistory: any[] = [];
  private model: string = 'deepseek-chat'; // DeepSeek multimodal model
  ...
  /**
   * Analyze screenshot using DeepSeek's vision capabilities
   */
  async analyzeScreen(screenshotPath: string): Promise<ScreenAnalysis> {
    try {
      logger.info(`Analyzing screenshot with DeepSeek: ${screenshotPath}`);
...
Provide a structured analysis that can help a voice-controlled automation agent interact with this screen.`
      ...
      logger.info('Screen analysis completed with DeepSeek');
      ...
      logger.error('DeepSeek screen analysis failed:', error);
      throw error;
    }
  }
  ...
  async interpretCommand(
    ...
    try {
      logger.info(`Interpreting command with DeepSeek: "${voiceCommand}"`);
...
Provide only the JSON array, no additional text.`;
      ...
      logger.info('Command interpretation completed with DeepSeek');

      // Parse JSON response
      const actions = this.parseActions(responseText);
      return actions;
    } catch (error) {
      logger.error('DeepSeek command interpretation failed:', error);
      throw error;
    }
  }

  /**
   * Get a conversational response from DeepSeek
   */
  ...
  private parseActions(responseText: string): AgentAction[] {
    try {
      ...
      logger.warn('Failed to parse actions from DeepSeek response, using fallback');
      return [
        {
          type: 'speak',
  ...
  resetConversation(): void {
    this.conversationHistory = [];
    logger.info('DeepSeek conversation history reset');
  }
}
